...
namespace DalOraInfra.DAL
{
    ...
    public class clDal : IDisposable
    {
        ...
        public clDal(IUnityContainer container)
        {
        ...
        public void ExecuteSP(string sSPName, DataTable dt)
        {
            OracleDataAdapter adapter = new OracleDataAdapter();
            ...
            try
            {
                ...
                cmd.Connection = conn;
                adapter.SelectCommand = cmd;
                string paramsStr = GetParamsString(cmd);
                _container.Resolve<ILogger>().Log(string.Format("clDal.ExecuteSP (with DataTable -using adapter Fill): Preparing to execute {0} with params {1}", sSPName, paramsStr), Category.Debug);
                var totalMsToExecute = StopwatchHelper.TimedAction(() => adapter.Fill(dt));
                _container.Resolve<ILogger>().Log(string.Format("clDal.ExecuteSP (with DataTable -using adapter Fill): Total ms to execute SP {0} is {1}", sSPName, totalMsToExecute), Category.Debug);
                adapter.Dispose();
            }
            catch (Exception ex)
            {
        ...
        public void ExecuteSP(string sSPName)
        {
            try
            {
                ...
                cmd.Connection = conn;
                string paramsStr = GetParamsString(cmd);
                _container.Resolve<ILogger>().Log(string.Format("clDal.ExecuteSP: Preparing to execute {0} with params {1}", sSPName,paramsStr), Category.Debug);
                var totalMsToExecute = StopwatchHelper.TimedAction(() => cmd.ExecuteNonQuery());
                _container.Resolve<ILogger>().Log(string.Format("clDal.ExecuteSP: Total ms to execute SP {0} is {1}", sSPName, totalMsToExecute), Category.Debug);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private string GetParamsString(OracleCommand cmd)
        {
            if (cmd == null)
                return string.Empty;
            ...
            foreach (OracleParameter param in cmd.Parameters)
            {
                ...
                {
                    sb.Append(string.Format("Param name: {0}, Value: {1}, ", param.ParameterName, param.Value.ToString()));
                }
            }
            return sb.ToString();
        }
